...
function constant(expr) {
    ...
    if (expr.operator === 'and' && typeof left === 'boolean')
        return left ? constant(expr.right) : false;
    if (expr.operator === 'or' && typeof left === 'boolean')
        return left ? true : constant(expr.right);
...
function expressionKey(expr) {
    ...
    if (expr.kind === 'binary')
        return `(${expressionKey(expr.left)}${expr.operator}${expressionKey(expr.right)})`;
    if (expr.kind === 'index')
...
function definitelyTerminates(statement) {
    ...
    if (statement.kind === 'while')
        return constant(statement.condition.expression) === true;
    if (statement.kind === 'choice')
        return statement.options.length > 0 && statement.options.every((option) => blockTerminates(option.body));
    return false;
}
function blockTerminates(statements) {
    return statements.some(definitelyTerminates);
}
function analyzeExpression(expr, file, out) {
...
function analyzeBlock(statements, file, out, reachable = true) {
    ...
    for (const statement of statements) {
        ...
        if (statement.kind === 'if') {
            ...
            if (statement.otherwise.length)
                analyzeBlock(statement.otherwise, file, out, canReach && !previousAlways);
            ...
            if (value === true)
                out.push(diagnostic(file, 'infinite-loop', 'warning', 'while の条件は常に真で、ループを抜ける文がありません', statement.condition));
            analyzeBlock(statement.body, file, out, canReach && value !== false);
        }
        else {
            for (const body of nested(statement))
                analyzeBlock(body, file, out, canReach);
        }
        if (canReach && definitelyTerminates(statement))
            canReach = false;
    }
}
...
function analyzeScript(script, file = 'current', externalGlobals = new Map(), externalCharacters = new Map()) {
    const out = [];
    try {
        (0, type_checker_1.checkTypes)(script, file, externalGlobals, externalCharacters);
    }
    catch (error) {
